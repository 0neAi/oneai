...
class PathaoApiClient {
  ...
  async loginAgent(agent) {
    try {
      ...
      let response;
      try {
        response = await fetch(`${this.baseUrl}/talaria/api/v1/issue-token`, {
          method: 'POST',
          headers: {
      ...
      if (!token) {
        throw new Error('No access token returned from Pathao login');
      }
      ...
      console.log(`✅ Agent ${agent.displayName} logged in`);
      return token;
    } catch (error) {
  ...
  async getToken(agent) {
    ...
    return await this.loginAgent(agent);
  }
  ...
  async fetchAgentOrders(agent) {
    ...
    try {
      ...
      let response;
      try {
        response = await fetch(`${this.baseUrl}/talaria/api/v1/user/delivery?page=1&limit=1000`, {
          headers: {
            Accept: 'application/json',
            'App-Version': this.appVersion,
      ...
      return orders.map((order) => ({
        ...order,
        status: normalizeAgentStatus(order.status),
        agentName: agent.id,
        agentDisplayName: agent.displayName,
        agentAssigned: agent.displayName || '',
        assigned: Boolean(agent.displayName && String(agent.displayName).trim())
      }));
    } catch (error) {
      console.error(`❌ Fetch error for ${agent.displayName}:`, error.message || error);
      return [];
    }
  }
  ...
  _extractOrders(response) {
    ...
    return orders.map((order) => ({
      ...
      price: parseFloat(order.amount || order.price || 0) || 0,
      ...
      quantity: parseInt(order.quantity || order.qty || 1, 10) || 1,
