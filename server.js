...
// Broker credit packages (credits -> USDT price)
const BROKER_CREDIT_PACKAGES = [
...
app.get('/broker/orders/hold', validateUser, async (req, res) => {
  try {
    const mineOnly = req.query.mine === 'true';
    const filter = { status: 'HOLD' };
    if (mineOnly) filter.user = req.user._id;
    const orders = await BrokerOrder.find(filter).sort({ createdAt: -1 });
    res.json({ success: true, orders });
  } catch (error) {
    console.error('Error fetching hold orders:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch hold orders' });
  }
});

app.get('/broker/orders/unassigned', validateUser, async (req, res) => {
  try {
    const mineOnly = req.query.mine === 'true';
...
app.post('/broker/orders', validateUser, async (req, res) => {
  try {
    ...
    res.status(201).json({ success: true, order: brokerOrder, brokerCredits: user.brokerCredits });
  } catch (error) {
...
app.put('/broker/orders/:id/status', validateUser, async (req, res) => {
  try {
    const { id } = req.params;
    const { status, note } = req.body;
    const allowedStatuses = ['PENDING', 'PICKUP', 'DELIVERED', 'CANCELLED', 'FAILED', 'RETURNED'];
    ...
    console.error('Error updating broker order status:', error);
    res.status(500).json({ success: false, message: 'Failed to update broker order status.' });
  }
});

app.post('/broker/orders/:id/track', validateUser, async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const trackingConsignmentId = consignmentId || order.consignmentId;

    if (!trackingPhone || !trackingConsignmentId) {
...
app.get('/broker/credits', validateUser, async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select('brokerCredits brokerSubscriptionTier brokerSubscriptionExpiresAt');
    if (!user) {
      return res.status(404).json({ success: false, message: 'User not found.' });
    }
    res.json({ success: true, credits: user.brokerCredits, subscriptionTier: user.brokerSubscriptionTier, subscriptionExpiresAt: user.brokerSubscriptionExpiresAt });
  } catch (error) {
    console.error('Error fetching broker credits:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch broker credits' });
  }
});

app.get('/broker/subscription/status', validateUser, async (req, res) => {
  try {
    const user = await User.findById(req.user._id).select('brokerSubscriptionTier brokerSubscriptionExpiresAt brokerCredits');
    if (!user) {
      return res.status(404).json({ success: false, message: 'User not found.' });
    }
    ...
    res.json({
      success: true,
      subscriptionTier: user.brokerSubscriptionTier,
      subscriptionExpiresAt: user.brokerSubscriptionExpiresAt,
      credits: user.brokerCredits
    });
  } catch (error) {
    console.error('Error fetching broker subscription status:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch broker subscription status' });
  }
});

app.get('/broker/credit-packages', validateUser, async (req, res) => {
  try {
    res.json({ success: true, packages: BROKER_CREDIT_PACKAGES });
  } catch (error) {
    console.error('Error fetching broker credit packages:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch credit packages' });
  }
});

app.post('/broker/credits/purchase', validateUser, async (req, res) => {
  try {
    const { packageId, paymentMethod, trxid, paymentTxId } = req.body;
    const paymentReference = (typeof trxid === 'string' && trxid.trim()) ? trxid.trim() : (typeof paymentTxId === 'string' ? paymentTxId.trim() : '');
    ...
    if (!paymentReference) {
      return res.status(400).json({ success: false, message: 'TRXID or payment reference is required.' });
    }
...
// Admin broker order endpoints
app.get('/admin/broker-orders', adminAuth, async (req, res) => {
  try {
    const orders = await BrokerOrder.find().populate('user', 'email phone').sort({ createdAt: -1 });
    res.json({ success: true, orders });
  } catch (error) {
    console.error('Error fetching admin broker orders:', error);
    res.status(500).json({ success: false, message: 'Failed to fetch broker orders.' });
  }
});
...
app.put('/admin/broker-orders/:id/status', adminAuth, async (req, res) => {
  try {
    ...
    const order = await BrokerOrder.findById(id);
    if (!order) {
      return res.status(404).json({ success: false, message: 'Broker order not found.' });
    }
    ...
    order.statusHistory.push({ status, note: note || `Admin updated status to ${status}` });
    await order.save();

    wss.clients.forEach(client => {
      if (client.readyState === WebSocket.OPEN && client.userID === order.user?.toString()) {
        client.send(JSON.stringify({
          type: 'broker-order-updated',
          order
        }));
    ...
    console.error('Error updating admin broker order status:', error);
    res.status(500).json({ success: false, message: 'Failed to update broker order status.' });
  }
});
...
app.post('/admin/broker-orders/fetch', adminAuth, async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Broker order fetch triggered successfully.' });
  } catch (error) {
    console.error('Error triggering broker order fetch:', error);
    res.status(500).json({ success: false, message: 'Failed to trigger broker order fetch.' });
  }
});

app.post('/admin/broker-orders/morning', adminAuth, async (req, res) => {
  try {
    await orderFetcher.morningWorkflow();
    res.json({ success: true, message: 'Morning broker workflow triggered successfully.' });
  } catch (error) {
    console.error('Error triggering morning broker workflow:', error);
    res.status(500).json({ success: false, message: 'Failed to trigger morning broker workflow.' });
  }
});

app.post('/admin/broker-orders/closing', adminAuth, async (req, res) => {
  try {
    await orderFetcher.closingProcess();
    res.json({ success: true, message: 'Closing broker workflow triggered successfully.' });
  } catch (error) {
    console.error('Error triggering closing broker workflow:', error);
    res.status(500).json({ success: false, message: 'Failed to trigger closing broker workflow.' });
  }
});
